...
-- ROUND 함수 : 반올림 값을 구하는 함수
/*
    ROUND(값, 자리수) : 반올림기준 위치 자리수
...
-- 고객별 평균 주문액을 검색
SELECT bookSale.clientNo, 
        ROUND(AVG(book.bookPrice*bookSale.bsQty)) "평균 주문액", -- 자릿수 생략(1자리까지 출력-기본설정값)
        ROUND(AVG(book.bookPrice*bookSale.bsQty),0) "1자리까지 출력", 
        ROUND(AVG(book.bookPrice*bookSale.bsQty),-1) "10자리 까지 출력",
        ROUND(AVG(book.bookPrice*bookSale.bsQty),-2) "100자리 까지 출력",
        ROUND(AVG(book.bookPrice*bookSale.bsQty),-3) "1000자리 까지 출력"
FROM book, bookSale
...
-- 서점에서 판매중인 도서 정보를 출력하시오. 단, 안드로이드 관련 제목이 한글인 도서는 영문으로 변환해서 검색
-- REPLACE() : 문자열을 치환하는 함수
...
-- 서울 출판사에서 출판한 도서의 도서명과 글자수 바이트수 출판사명 출력
SELECT bookName, LENGTH(bookName) 도서명길이, LENGTHB(bookName) 도서명크기, pubName
FROM book B, publisher P
WHERE B.pubNo = P.pubNo AND P.pubName = '서울 출판사';

-- SUBSTR(문자열, 시작위치, 길이) : 지정한 길이만큼의 문자열을 반환하는 함수
...
-- INSTR : 문자열에서 지정된 문자열 검색해서 위치 반환, 첫번째 출현하는 위치를 반환
SELECT INSTR('초등학생, 고등학생, 대학생','학생',1) -- 반환값은 학생글자가 시작되는 위치
FROM DUAL;

SELECT INSTR('초등학생, 고등학생, 대학생','학생',1,2) -- 반환값은 두번째 학생글자가 시작되는 위치
FROM DUAL;

-- 문자열 처리할때 주의할 사항 : 공백을 제대로 처리해야 함
-- 문자열 처리시 앞뒤 공백은 제거 후 사용
-- TRIM() 문자열 공백 처리
SELECT TRIM('    대한민국   ')
FROM DUAL;

-- 공백이 아닌 다른 문자 제거
SELECT TRIM(BOTH '*' FROM '****대한민국****') --양쪽제거
FROM DUAL;
SELECT TRIM(LEADING '*' FROM '****대한민국****') --왼쪽제거
FROM DUAL;
SELECT TRIM(TRAILING '*' FROM '****대한민국****') --오른쪽제거
FROM DUAL;
...
SELECT SYSDATE-1 어제, SYSDATE+1 내일, SYSDATE+2 모레 FROM DUAL;

SELECT SYSDATE+7 일주일후 FROM DUAL;

-- 경과 단위를 MONTH로 처리 : ADD_MONTHS(날짜, 경과월)
...
-- 순위 관련 함수는 OVER()와 같이 사용함
-- RANK() : 값의 순위 반환(동일값이 있으면 다음 순위는 동일 값 만큼의 수가 증가함)
-- DENSE_RANK() : 값의 순위 반환(동일값이 있어도 다음 순위는 1 증가함)
...
SELECT prdName, color, SUM(amount) AS "금액합계"
FROM cubeTBL
GROUP BY ROLLUP(prdName,color) -- 기준컬럼 : prdName
ORDER BY prdName, color;

SELECT prdName, color, SUM(amount) AS "금액합계"
FROM cubeTBL
GROUP BY ROLLUP(color,prdName) -- 기준컬럼 : color
ORDER BY prdName, color;
