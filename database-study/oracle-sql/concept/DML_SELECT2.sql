-- DML SELECT - GROUP BY HAVING CREATE~SELECT

/*
  GROUP BY에서 사용가능한 GROUP 함수
    SUM() : 합계
    AVG() : 평균
    ...
    GROUP BY <속성>
    그룹 질의를 기술할 때 사용
    특정 열로 그룹화(값이 동일한 레코드들끼리의 집합)한 후 각 그룹에 대해 한 행씩 질의 결과 생성
    주의!!
...
FROM bookSale
WHERE clientNo = '2'
GROUP BY clientNo; -- 기준이 clientNo 이므로 해당 컬럼은 select절에 사용 가능

-- GROUP BY
-- 도서 판매 테이블에서 도서번호로 주문수량 합계 검색
...
--------------------------------------------------------------------------------
-- 도서테이블에서 출판사별 도서 중 도서 가격이 25000 이상인 도서의 수량의 합계를 검색, 단 반환되는 결과는 도서가 2권 이상일 경우 반환한다
-- 도서 가격이 25000 이상인 도서 : WHERE
-- 단 반환되는 결과는 도서가 2권 이상 : 집계함수 결과로 확인해야 함 => HAVING절 사용
...
-- 도서테이블에서 출판사별 도서의 총 재고수량을 검색하고 재고가 20건 이상인 결과만 출력
...
-- book 테이블의 도서 정보 중 2019년 이후에 출판된 도서의 정보를 새로운 테이블 newBook으로 생성
...
-- 구조만 필요한 경우
DELETE FROM newbook; -- 조건절 없으므로 모든 데이터가 삭제
