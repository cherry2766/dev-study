...
-- 사용자 권한 관리 주체 : DBA
-- GRANT TO : 데이터베이스 객체에 대한 권한 부여하는 질의어
...
-- CONNECT 롤 : SESSION 관련/CREATE 관련 권한 (로그인 권한)
-- RESOURCE 롤 : 사용자 객체 생성 및 접근 권한에 해당되는 롤 (테이블 만들 수 있는 권한)
...
-- DBMS에 접속하기 위한 권한 부여해야 접속/객체 활용 가능
-- 일반 사용자 필수 ROLL 부여
-- DBMS에 접속한 상태를 SESSION이라고 함
...
ALTER USER NEW_USER2 IDENTIFIED BY "1111";
...
-- ALL TABLES : 현재 사용자 자신이 생성한 객체와 다른 사용자가 만든 객체 중 권한이 있는 객체 검색
-- 사용자 권한에 따라 접근 권한이 있는 객체
...
-- 사용자 계정 삭제(오라클에서는 DB삭제임 : 신중하게 삭제를 진행해야 함)
-- DROP USER 계정명
-- DROP USER 계정명 CASECADE; -- 제약까지 모두 삭제

DROP USER NEW_USER; -- 접속되어 있지 않은 사용자 계정 삭제 바로 진행됨
DROP USER NEW_USER1; -- 접속되어 있는 사용자 계정 삭제 불가능
