...
SELECT * 
FROM book,publisher
WHERE book.pubNo = publisher.pubNo -- 조인조건
    AND bookNo = '1001';
...
-- 주문테이블에서 확인 가능한 고객의 정보 : 주문한 적이 있는 고객의 주문정보와 고객정보
SELECT *
FROM bookSale
    INNER JOIN client
...
-- 양쪽 테이블에 공통되는 열에 대해서는 테이블명을 표기해서 모호성을 없애야 함
-- 테이블명 없으면 오류
-- ORA-00918: 열의 정의가 애매합니다 clientNo가 두 테이블 모두에 컬럼명으로 있음(select, on, where 절 모두에 해당됨)
/*
...
-- 테이블명 추가가 열의 정확한 위치를 알려주므로 성능이 향상됨
SELECT DISTINCT client.clientNo, client.clientName, client.clientAddress
FROM client
    INNER JOIN bookSale
    ON client.clientNo = bookSale.clientNo;
    
-- 테이블에 별명 추가 FROM 절과 JOIN 절에서 : 가장 많이 사용함
...
-- bookSale 테이블 : 한명의 고객은 여러권의 책을 주문할 수 있다 / 한 권의 책은 여러 고객에게 주문될 수 있다
-- 고객 - N -> 주문 < - N 책
-- 기본키 : bsNo
-- 외래키 : clientNo, bookNo
...
-- 주문테이블만으로 위 질의를 진행하려면 고객명과 도서명은 검색 불가능 함
...
-- 도서를 주문한 고객명, 주문정보, 도서명 검색/ 주문번호를 기준으로 오름차순 정렬
...
-- GROUP BY 된 경우 집계함수가 아닌 일반 컬럼은 GROUP BY 조건이여야 함
SELECT C.clientNo, C.clientName, SUM(BS.bsQty) AS "총 주문 수량"
FROM bookSale BS
    INNER JOIN client C
    ON C.clientNo = BS.clientNo
GROUP BY C.clientNo, C.clientName -- 그룹 내에 또다른 그룹을 구성하라는 조건
ORDER BY "총 주문 수량" DESC;

-- WHERE 절 추가
-- 2019년부터~ 현재까지 판매된 도서의 주문일, 고객명, 도서명, 도서가격, 주문수량, 주문액(주문수량*도서가격) 계산하여 출력
-- 주문,고객,도서
SELECT BS.bsDate AS 주문일, C.clientName AS 고객명, B.bookName AS 도서명, B.bookPrice AS 도서가격, BS.bsQty AS 주문수량, BS.bsQty*B.bookPrice AS 주문액
FROM bookSale BS
    INNER JOIN client C ON C.clientNo = BS.clientNo
    INNER JOIN book B ON B.bookNo = BS.bookNo
...
-- 한번도 주문하지 않은 고객도 존재함
-- INNER JOIN은 주문한 고객정보만 결합 - 주문하지 않은 고객은 반영하지 않음
-- 주문하지 않은 고객 정보까지 표현 : OUTER JOIN
-- 주문하지 않은 고객 : 고객정보는 저장된 정보 표현, 주문정보는 NULL로 표현

-- 주문하지 않은 고객 정보가 client 테이블에 있음
SELECT *
FROM client C
...
-- 주문은 했지만 고객정보에 없는 주문을 포함한 결과 출력 : 고객정보가 없는 주문은 잘못된 주문(삭제해야할 내용임)
-- 비회원에 대한 의미는 아님
SELECT *
FROM client C
...
-- INNER JOIN 방법
SELECT *
FROM client C, bookSale BS -- 조인할 테이블 나열
WHERE C.clientNo = BS.clientNo; -- 조인조건 일반조건은 AND 연결 후 표현

-- OUTER JOIN 방법 : 조건절에서 (+)기호로 표현
-- 확장되어야 할 테이블에 (+)기호로 표현
-- client 테이블 데이터는 모두 표현해야 하고 bookSale에는 없는 경우 확장해서 NULL 값 표현
SELECT *
