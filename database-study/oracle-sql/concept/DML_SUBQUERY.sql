...
-- 서브쿼리 종류 : 스칼라 서브쿼리(Scalar Subquery)
...
-- 도서가격이 25000원 이상인 도서 중 판매된 도서에 대해 도서별로 도서명, 가격, 총 판매수량, 총 판매액 검색, 총 판매액 기준 내림차순 정렬
-- 인라인뷰 서브쿼리 사용

SELECT * FROM book;

-- 도서테이블에서 질의에 필요한 컬럼과 레코드만 추출(VIEW로 사용할 예정)
SELECT bookNo, bookName, bookPrice
FROM book
WHERE bookPrice >=25000

-- 도서가격이 25000원 이상인 도서 중 판매된 도서에 대해 도서별로 도서명, 가격, 총 판매수량, 총 판매액 검색, 총 판매액 기준 내림차순 정렬
-- 인라인뷰 서브쿼리와 일반 테이블간의 조인
SELECT book.bookName, book.bookPrice, SUM(bookSale.bsQty) AS 총판매수량, SUM(book.bookPrice*bookSale.bsQty) AS 총판매액
FROM (SELECT bookNo, bookName, bookPrice
      FROM book
      WHERE bookPrice >=25000) book, bookSale -- FROM절에서 뷰와 테이블명을 나열하면 INNER JOIN이 진행됨
WHERE book.bookNo = bookSale.bookNo -- 조인 조건
